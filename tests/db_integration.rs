use anyhow::Result;
use clam::{Interaction, Post, UserPreferenceCalculator};
...
async fn test_database_integration() -> Result<()> {
    ...
    for result in reader.records().take(1000) {
        ...
        let embedding = calc.calculate_preference_vector(&interactions).await?.unwrap();


        sqlx::query("INSERT INTO posts (id, title, body, embedding) VALUES ($1, $2, $3, $4::vector)")
    ...
    let user_history = posts.iter()
        .filter(|p| p.title.to_lowercase().contains("tech") || p.body.to_lowercase().contains("internet"))
        .take(3)
    ...
    let user_pref_vector = calc.calculate_preference_vector(&user_history).await?.unwrap();
